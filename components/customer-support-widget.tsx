...
export function CustomerSupportWidget() {
  ...
  useEffect(() => {
    if (!visitorId) return;
    const heartbeat = window.setInterval(() => void register(), 15_000);
    return () => window.clearInterval(heartbeat);
  }, [visitorId, pathname]);

  useEffect(() => {
    if (!visitorId) return;
    const eventSource = new EventSource(`/api/support/stream?visitorId=${encodeURIComponent(visitorId)}`);
  ...
  return (
    <section className="fixed bottom-4 right-4 z-50 flex flex-col items-end gap-3 sm:bottom-6 sm:right-6">
      {isOpen && (
        <div className="w-[calc(100vw-2rem)] overflow-hidden rounded-3xl border bg-white shadow-2xl sm:w-96 dark:border-neutral-700 dark:bg-neutral-900">
          <header className="flex items-center justify-between bg-neutral-900 px-5 py-4 text-white">
            <div><p className="text-sm font-semibold">Canlı müşteri desteği</p><p className="mt-1 text-xs text-neutral-300">🟢 Temsilciler çevrimiçi</p></div>
            <button onClick={() => setIsOpen(false)} aria-label="Kapat"><XMarkIcon className="h-5 w-5" /></button>
          </header>
          <div ref={transcriptRef} className="flex max-h-80 flex-col gap-3 overflow-y-auto px-4 py-5">
            {messages.map((message) => <div key={message.id} className={clsx('flex flex-col', message.author === 'customer' ? 'items-end' : 'items-start')}><p className={clsx('max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-6', message.author === 'customer' ? 'bg-blue-600 text-white' : 'bg-neutral-100 dark:bg-neutral-800 dark:text-white')}>{message.text}</p>{message.attachmentUrl && <a href={message.attachmentUrl} target="_blank" rel="noreferrer" className="mt-1 text-xs text-blue-600">📎 {message.attachmentName}</a>}<span className="mt-1 text-[11px] text-neutral-500">{new Date(message.createdAt).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}</span></div>)}
            {agentTyping && <p className="text-xs text-neutral-400">Temsilci yazıyor...</p>}
          </div>
