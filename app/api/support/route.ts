...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    ...
    conversations: admin ? snapshot.conversations : snapshot.conversations.filter((conversation) => conversation.id === conversationId),
    messages: visitorId ? snapshot.messages.filter((message) => message.conversationId === conversationId) : admin ? snapshot.messages : [],
  });
}
...
export async function POST(request: Request) {
  ...
  const admin = isAdminRequest(request);
  if (!admin && (body.author === 'agent' || body.action === 'status' || body.action === 'read' || body.action === 'agent_presence' || (body.action === 'typing' && body.author !== 'customer'))) {
    return NextResponse.json({ error: 'Yetkili girişi gerekli.' }, { status: 401 });
  }
  if (body.action === 'agent_presence') return NextResponse.json({ agentOnline: await setAgentOnline(Boolean(body.online)) });
